...
for k=1:size(I0,1)
 ...
 %titoli e labels
   str = ['TRAIETTORIE per b=',num2str(b)];
   title(str);
   xlabel('x1');
   ylabel('x2');
   zlabel('x3');
...
%  
 ...
 %titoli e labels
   str = ['TRAIETTORIE  con piani di poincare con dZ=0   b=',num2str(b) ];
   title(str);
   xlabel('x1');
   ylabel('x2');
   zlabel('x3');
...
[ t,cycle,TE,XE] = ComputeCycle(@osc,@HRxmaxy,@HRjac,1e-2,[1,1000],I0(1,:));
